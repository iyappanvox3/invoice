...
class CustomHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith('/pay'):
            ...
            pay_html_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'pay.html')
    ...
    def do_POST(self):
        ...
        if self.path == '/create-order':
            ...
            try:
                ...
                site_base = config.get('base_url', 'http://localhost:8002').strip().rstrip('/')
                payment_url = f'{site_base}/pay?token={token}'

                # Send email to customer
                self.send_payment_email(smtp_user, smtp_password, customer_email,
                                        invoice_num, total, payment_url)
        ...
        if self.path == '/submit-details':
            ...
            try:
                ...
                if not company or not address or not city or not email:
                    self.send_error_response(400, 'Required billing details are missing.')
                    return
                ...
                if pdf_base64:
                    ...
                    if pisa_status.err:
                        self.send_error_response(500, f'Error generating PDF: {pisa_status.err}')
                        return
                ...
                self.send_invoice_email(smtp_user, smtp_password, recipients, order.get('invoice_num', 'Invoice'), final_pdf_base64)
        ...
        if self.path == '/send-email':
            ...
            try:
                ...
                if not pdf_base64:
                    ...
                    return
                if not recipients:
                    self.send_error_response(400, 'Please enter at least one recipient email address.')
                    return
                ...
                if not smtp_user or smtp_user == 'your-email@gmail.com':
                    self.send_error_response(400, 'Gmail address not configured in config.json.')
                    return
                if not smtp_password or smtp_password == 'xxxx-xxxx-xxxx-xxxx':
                    self.send_error_response(400, 'Gmail App Password not configured in config.json.')
                    return

                self.send_invoice_email(smtp_user, smtp_password, recipients, invoice_num, pdf_base64)
                self.send_success_response({'status': 'success', 'message': f"Successfully sent invoice to: {', '.join(recipients)}"})
        ...
        if self.path == '/initiate-ccavenue':
            ...
            try:
                ...
                if not merchant_id or not access_code or not working_key:
                    self.send_error_response(400, 'CCAvenue credentials not configured in config.json.')
                    return
        ...
        if self.path == '/ccavenue-response':
            ...
            try:
                ...
                if not encResp:
                    ...
                    self.send_header('Location', '/pay?payment_status=failed&error=no_response_payload')
                    self.end_headers()
                    return
                ...
                if not order_id:
                    self.send_response(303)
                    self.send_header('Location', '/pay?payment_status=failed&error=missing_order_id')
                    self.end_headers()
                    return
                ...
                if not order:
                    self.send_response(303)
                    self.send_header('Location', '/pay?payment_status=failed&error=order_not_found')
                    self.end_headers()
                    return
                ...
                if order_status == 'Success':
                    ...
                    self.send_response(303)
                    self.send_header('Location', f'/pay?token={order_id}&payment_status=success')
                    self.end_headers()
                else:
                    self.send_response(303)
                    err_msg = urllib.parse.quote(failure_message or f'Payment {order_status}')
                    self.send_header('Location', f'/pay?token={order_id}&payment_status=failed&error={err_msg}')
                    self.end_headers()

            except Exception as e:
                self.send_response(303)
                err_msg = urllib.parse.quote(str(e))
                self.send_header('Location', f'/pay?payment_status=failed&error={err_msg}')
                self.end_headers()
            return
    ...
    def send_payment_email(self, smtp_user, smtp_password, recipient,
                           invoice_num, total, payment_url):
